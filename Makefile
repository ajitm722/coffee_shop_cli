...
help:
	...
	@echo "  make view-orders     - (Generate server and client binaries first) View all orders"
	@echo "  make place-order     - (Generate server and client binaries first) Places a coffee order (Enter details at runtime)"
	@echo "  make get-order       - (Generate server and client binaries first) Get details of a specific order by ID (Enter details at runtime)"
